...
@RunAsClient
@ServerSetup(ExternalVaultPasswordTestCase.ExternalVaultPasswordSetup.class)
public class ExternalVaultPasswordTestCase {

    private static Logger LOGGER = Logger.getLogger(ExternalVaultPasswordTestCase.class);
    ...
    public static final String VAULT_PASSWORD = "rightVaultPass";
    ...
    public static WebArchive appDeploymentCahce() {
        ...
        war.addClass(CheckServlet.class);
        war.addClass(ExternalPasswordProvider.class);
        war.addAsWebInfResource(Utils.getJBossWebXmlAsset("other"), "jboss-web.xml");
        war.addAsManifestResource(Utils.getJBossDeploymentStructure("org.picketbox"), "jboss-deployment-structure.xml");
        LOGGER.debug(war.toString(true));

        Utils.saveArchiveToFolder(war, "/home/fbogyai/workspace");
        return war;
    }
    ...
    /**
     * TestingTesting {CLASS}classname[:ctorargs]' option password where the
     * '[:ctorargs]' is an optional string delimited by the ':' from the
     * classname that will be passed to the classname ctor. The ctorargs itself
     * is a comma delimited list of strings.
     */
    @Test
    @Ignore("WFLY-3789")
    public void testClassPassword() throws Exception {
    ...
    public String getVaultedPassword() throws Exception {
        ...
        URI vaultCheckServlet = new URI(url.toExternalForm() + CheckServlet.SERVLET_PATH + "?" + CheckServlet.VAULTED_PASS + "="
                + VAULTED_PROPERTY);
